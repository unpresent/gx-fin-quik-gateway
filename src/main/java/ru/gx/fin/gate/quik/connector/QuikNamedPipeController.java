...
import ru.gx.fin.gate.quik.api.QuikNamedPipeMessenger;
...
class QuikNamedPipeController implements QuikConnectionApi, QuikNamedPipeMessenger {
     ...
     */
    @Override
    public synchronized void init(@Nullable final String pipeName) {
    ...
    protected synchronized void connectInternal() throws IOException, InterruptedException {
        ...
        try {
            ...
            log.info("is connecting to {}", this.pipeFile != null ? this.pipeFile.getName() : null);
    ...
    public synchronized String receiveMessage() throws QuikConnectorException, IOException {
        ...
        if (this.state != ConnectorState.WaitCommand) {
            ...
            throw new QuikConnectorException(QuikNamedPipeMessenger.class.getSimpleName() + " can't receiveMessage because it is " + this.state + "!");
        }
        if (this.currentPipe == null) {
            log.info("can't receiveMessage() because this.currentPipe is null!");
            throw new QuikConnectorException(QuikNamedPipeMessenger.class.getSimpleName() + " can't receiveMessage because this.currentPipe is null!");
        }
    ...
    public synchronized void sendMessage(String message) throws QuikConnectorException, IOException {
        ...
        if (this.state != ConnectorState.WaitCommand) {
            ...
            throw new QuikConnectorException(THIS_NAME + " can't sendMessage() because it is " + this.state + "!");
        }
        if (this.currentPipe == null) {
            log.info("Can't sendMessage() because this.currentPipe is null");
            throw new QuikConnectorException(THIS_NAME + " can't sendMessage() because this.currentPipe is null!");
        }
